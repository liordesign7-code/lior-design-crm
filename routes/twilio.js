...
const WEEKDAY_HOURS = {
  0:{start:8,end:19},1:{start:8,end:19},2:{start:8,end:19},
  3:{start:8,end:19},4:{start:8,end:19},5:{start:8,end:12},
};
...
function isBusinessHours(date) {
  ...
  const hours = HOLIDAY_EVES.includes(dateStr) ? {start:8,end:12} : WEEKDAY_HOURS[day];
...
function getMsg(key) {
  try { const msgs = db.getMessages(); return (msgs && msgs[key]) || DEFAULT_MESSAGES[key] || ''; }
...
async function getOrCreateLead(phone, source) {
  ...
  if (!lead) {
    lead = await db.addLead({ name:'לקוח מהחטאת שיחה', phone:clean, source, status:'new', waitingForName:true, missedCallAt:Date.now() });
  }
  return lead;
}
...
router.post('/missed-call', async (req, res) => {
  ...
  const lead = await getOrCreateLead(phone, 'טלפון');
  await db.updateLead(lead.id, { missedCallAt:Date.now(), waitingForName:true });
  await db.addLog(lead.id, '📞 שיחה שלא נענתה – ' + getNow().toLocaleTimeString('he-IL'));
  if (!isBusinessHours()) {
...
function scheduleFollowups(leadId, phone) {
  setTimeout(async () => {
    try {
      ...
      await db.addLog(leadId, '💬 הודעה שנייה נשלחה');
    } catch(e) {}
  }, 5*60*1000);
  ...
  setTimeout(async () => {
    try {
      ...
      await db.addLog(leadId, '💬 הודעת המתנה נשלחה');
...
router.post('/incoming-wa', async (req, res) => {
  ...
  if (!lead) {
    await db.addLead({ name:'ליד מ-WhatsApp', phone, source:'וואטסאפ', status:'new', notes:'💬 '+body.slice(0,100) });
  ...
  if (lead.waitingForName && body.length<=30 && !body.includes('http') && !/^\d+$/.test(body)) {
    await db.updateLead(lead.id, { name:body, waitingForName:false });
    await db.addLog(lead.id, '✅ שם: '+body);
    await sendMsg(phone, getMsg('gotName').replace('{name}',body)).catch(()=>{});
    if (OLGA_PHONE) await sendMsg(OLGA_PHONE, '✅ שם: '+body+'\n📞 '+phone).catch(()=>{});
    return;
  }
  await db.addLog(lead.id, '💬 '+body.slice(0,100));
